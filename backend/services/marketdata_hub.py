...
SYMBOL_TOKEN_RE = re.compile(r"^(NYSE|NASDAQ):([A-Z0-9][A-Z0-9._-]{0,40})$")
...
class MarketDataHub:
    ...
    async def start(self) -> None:
        async with self._lock:
            ...
            # Auto-subscribe to supported markets
            for m in US_MARKETS:
    ...
    def finnhub_stream_status(self) -> str:
        if not self._finnhub_ws.enabled:
            return "disabled"
        return "connected" if self._finnhub_ws.connected else "disconnected"
