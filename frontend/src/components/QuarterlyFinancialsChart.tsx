...
function fmtUsdBn(value: number): string {
...
export function QuarterlyFinancialsChart({ symbol }: { symbol: string }) {
  ...
  const rows = useMemo(
    () =>
      data.map((r) => ({
        ...r,
        revenueBn: toBillions(Number(r.revenue || 0)),
        netBn: toBillions(Number(r.net_profit || 0)),
      })),
    [data],
  );
  ...
  return (
    <div className="rounded border border-terminal-border bg-terminal-panel p-3">
      <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
        <div className="text-sm font-semibold text-terminal-accent">Quarterly Financials (USD)</div>
